...
from scripts.data.genomic_data_loader import GenomicDataLoader  # noqa: E402
...
class GenomicEvaluator:
    ...
    def run(self, config_path):
        ...
        for run_idx, run in enumerate(runs.values()):
            ...
            repitions = run.eval.get('repetitions', run.eval.get('repitions', None))
            if repitions is None:
                raise ValueError(f"Missing required 'repetitions' (or legacy 'repitions') key for {eval_mode} mode in config.")
